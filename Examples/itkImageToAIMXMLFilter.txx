...
::GenerateXMLFromContours()
{
  ...
  // Constants for the volume exposure
  const char* CalculationUID = "1.2.826.0.1.3680043.2.1143.915446522238796896"
    "1849280069476997485";
  const char* EmptyString = "";

  // Casting the volume to a string
  std::string strVolume = boost::lexical_cast<std::string>(m_Volume);
  ...
  // Setup the volume in a calculationCollection
  curNode = xmlNewChild(rootNode, NULL, BAD_CAST "calculationCollection",
                        NULL);
  curNode = xmlNewChild(curNode, NULL, BAD_CAST "Calculation", NULL);
  xmlNewProp( curNode,
              BAD_CAST "id",
              BAD_CAST DefaultNumber );
  xmlNewProp( curNode,
              BAD_CAST "uid",
              BAD_CAST CalculationUID );
  xmlNewProp( curNode,
              BAD_CAST "description",
              BAD_CAST "Volume of the Lesion" );
  xmlNewProp( curNode,
              BAD_CAST "mathML",
              BAD_CAST EmptyString );
  xmlNewProp( curNode,
              BAD_CAST "codeValue",
              BAD_CAST "Volume" );
  xmlNewProp( curNode,
  ...
  xmlNewProp( curNode,
              BAD_CAST "codingSchemeVersion",
              BAD_CAST EmptyString );
  curNode = xmlNewChild(curNode, NULL, BAD_CAST "CalculationResult", NULL);
  xmlNewProp( curNode,
              BAD_CAST "id",
              BAD_CAST DefaultNumber );
  xmlNewProp( curNode,
              ...
              BAD_CAST "Volume" );
  xmlNewProp( curNode,
              BAD_CAST "numberOfDimensions",
              BAD_CAST "1" );
  xmlNewProp( curNode,
              BAD_CAST "unitOfMeasure",
              BAD_CAST "mm^3" );
  curNode = xmlNewChild(curNode, NULL, BAD_CAST "dataCollection", NULL);
  curNode = xmlNewChild(curNode, NULL, BAD_CAST "Data", NULL);
  xmlNewProp( curNode,
              BAD_CAST "id",
              BAD_CAST DefaultNumber );
  xmlNewProp( curNode,
              BAD_CAST "value",
              BAD_CAST strVolume.c_str() );
...
::GetCurrentTimeInAIMFormat() const
{
  ...
  oss << 1900+timeinfo->tm_year << "-" 
      << std::setw(2) << setfill('0') << timeinfo->tm_mon + 1 << "-"
